    ...
    gameControl.onAdd = function(map) {
      ...
      answerInput.addEventListener('keypress', async function(e) {
        if (e.key === 'Enter' && current_city && !player_guessed) {
          ...
          llava_analysis = await getLLaVAAnalysis();
          llavaDisplay.textContent = llava_analysis;
          updateGameState();
        }
      });
      ...
      L.DomEvent.on(nextRoundButton, 'click', async function() {
        ...
        if (round > 10) {
          ...
          // Show markers for all visited cities
          selected_cities.forEach((cityIdx, index) => {
            const city = cities[cityIdx];
            const marker = L.marker([city[2], city[3]]).addTo(map);
            marker.bindPopup(`Round ${index + 1}: ${city[1]}, ${city[0]}`);
          });
          ...
          // Add list of visited cities
          var citiesList = L.DomUtil.create('div', 'cities-list', gameOverDiv);
          citiesList.style.marginBottom = '20px';
          citiesList.style.textAlign = 'left';
          citiesList.style.maxHeight = '200px';
          citiesList.style.overflowY = 'auto';

          var citiesListTitle = L.DomUtil.create('h3', 'cities-list-title', citiesList);
          citiesListTitle.textContent = 'Cities Visited:';
          citiesListTitle.style.marginBottom = '10px';

          var citiesListContent = L.DomUtil.create('ul', 'cities-list-content', citiesList);
          selected_cities.forEach((cityIdx, index) => {
              const city = cities[cityIdx];
              const listItem = L.DomUtil.create('li', 'city-item', citiesListContent);
              listItem.textContent = `Round ${index + 1}: ${city[1]}, ${city[0]}`;
